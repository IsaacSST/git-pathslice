...
For example, a source commit might change both `src/pump.rs` and `specs/pump.md`.
The `specs` slice exports only the change to `specs/pump.md`, retaining the
...
the destination's version of `src/pump.rs`.

Merge the specification PR through the usual review process. The implementation
...
deletions.
Path selection does not check dependencies between specification and
